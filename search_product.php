...
<div class="container-fluid pt-5">
    ...
    <div class="row px-xl-5 pb-3">
        ...
        if(isset($_POST['search_query'])) {
            $_SESSION['search_query'] = $_POST['search_query']; // Lưu từ khóa tìm kiếm vào session
        }

        $searchQuery = isset($_SESSION['search_query']) ? '%' . $_SESSION['search_query'] . '%' : '%';
        $query = "SELECT * FROM products WHERE name LIKE :key LIMIT :offset, :recordsPerPage";
        $stmt = $conn->prepare($query);
        $stmt->bindParam(':key', $searchQuery);
                ...
                <div class="col-lg-4 col-md-6 col-sm-12 pb-1">
                    <div class="card product-item border-0 mb-4">
                        <div class="card-header product-img position-relative overflow-hidden bg-transparent border p-0">
                            <img class="img-fluid w-100" style="height: 280px; width: 100px;" src="http://localhost/clothingstore/admin/view/product/<?php echo $row['img']; ?>" alt="<?php echo $row['name']; ?>">
                        </div>
